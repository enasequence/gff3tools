...
A lazy, typed provider system that injects shared context into validators and fixes via `@InjectContext`-annotated fields. Providers are auto-discovered by ClassGraph and resolved lazily on first access.
...
`ValidationContext` provides typed, lazily-resolved providers accessible to all validators via `@InjectContext`-annotated fields. No validation method signatures change.
...
public interface ContextProvider<T> {
    ...
    Class<T> type();                    // value type (used as registration key)
}
```

`ValidationContext` holds a `Map<Class<?>, ContextProvider<?>>` registry. Key methods: `get(providerClass)`, `register(class, provider)`.

## Delivered Providers

| Provider | Replaces |
|----------|---------|
| `OntologyClientProvider` | Per-validator `ConversionUtils.getOntologyClient()` calls |

## Integration Points

- **`@InjectContext`**: Annotation on `ValidationContext` fields — `ValidationRegistry` injects the context instance via reflection during build
...
public void validate(GFF3Annotation annotation, int line) throws ValidationException {
    OntologyClient client = context.get(OntologyClient.class);
}
```
...
**Class-level injection over method parameters**: Adding `ValidationContext` as a third parameter would require changing every validator signature and the reflection-based dispatch in `ValidationEngine`. Field injection via `@InjectContext` is purely additive.

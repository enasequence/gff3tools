...
public class GFF3ToFFConverter {
  ...
  public static void main(String[] args) throws Exception {
    ...
    try (BufferedReader bufferedReader = Files.newBufferedReader(Path.of(filename))) {
      ...
      // new EmblEntryWriter(new
      // GFF3Mapper().mapGFF3ToEntry(recordSet).get(0)).write(ffWriter); // existing
  ...
  public Entry convertRecordSetToEntry(GFF3RecordSet recordSet) throws Exception {
    ...
    records.stream()
        .map(record -> {
          ...
          if (feature == null)
            throw new RuntimeException("Invalid Feature " + record.getType());
          return feature;
        })
        .forEach(entry::addFeature);

    // update locations
    // Rule: The location of parent will be the join of locations of all children,
    // complemented if strand is "-"
  ...
  private Feature convertRecordToFeature(GFF3Record record) {
    ...
    if (!featureMap.containsKey(record.getType()))
      return null;
    ...
    // Add location - ignoring strand in location because it is handled at the
    // parent level

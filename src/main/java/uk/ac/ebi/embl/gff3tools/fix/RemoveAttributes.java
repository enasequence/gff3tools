...
public class RemoveAttributes {
    ...
    public GFF3Feature fixFeature(GFF3Feature feature) {
        if (!feature.getName().equalsIgnoreCase(GFF3Anthology.OLD_SEQUENCE_FEATURE_NAME)) {
            Map<String, Object> updatedAttributes = new HashMap<>(feature.getAttributes());
            updatedAttributes.remove(GFF3Attributes.CITATION);
            updatedAttributes.remove(GFF3Attributes.COMPARE);

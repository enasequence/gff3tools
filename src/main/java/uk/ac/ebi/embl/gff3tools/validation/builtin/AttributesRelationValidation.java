...
public class AttributesRelationValidation extends Validation {
    ...
    private static final List<String> ALLOWED_CIRCULAR_FEATURES =
            List.of(OntologyTerm.CDS.ID, OntologyTerm.TRNA.ID, OntologyTerm.MRNA.ID);
    ...
    public void validateCircularRNAAttribute(GFF3Feature feature, int line) throws ValidationException {
        ...
        String circularAttr =
                FeatureMapping.getGFF3Attribute(GFF3Attributes.CIRCULAR_RNA).orElse(null);
        if (circularAttr == null || soId == null) return;

        if (feature.hasAttribute(circularAttr)) {

...
public class FeatureSpecificValidation extends Validation {
    ...
    @ValidationMethod(rule = "OPERON_FEATURE", type = ValidationType.ANNOTATION)
    public void validateOperonFeatures(GFF3Annotation annotation, int line) throws ValidationException {
        ...
        for (Map.Entry<String, List<GFF3Feature>> entry : groupedByOperon.entrySet()) {
            ...
            if (group.size() == 1) {
                ...
                throw new ValidationException(line, SINGLE_OPERON_MESSAGE.formatted(feature.getName(), operonValue));
            } else {
                throw new ValidationException(line, MULTIPLE_OPERON_MESSAGE.formatted(group.size(), operonValue));
            }
        }
    }

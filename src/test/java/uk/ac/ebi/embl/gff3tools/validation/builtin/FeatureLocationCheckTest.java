...
public class FeatureLocationCheckTest {
    ...
    private GFF3Annotation annotationWithFeature(GFF3Feature feature) {
    ...
    @Test
    public void testValidateFeatureEndEqualsSequenceLengthSuccess() throws Exception {
        long seqLen = 1000L;
        injectLookupReturning(SEQ_ID, seqLen);
        GFF3Feature feature = TestUtils.createGFF3Feature("gene", SEQ_ID, 1L, seqLen, Map.of());
        assertDoesNotThrow(() -> check.validateFeatureEndWithinSequence(annotationWithFeature(feature), 1));
    }

    @Test
    public void testValidateFeatureEndExceedsSequenceLengthFailure() throws Exception {
        long seqLen = 1000L;
        injectLookupReturning(SEQ_ID, seqLen);
        GFF3Feature feature = TestUtils.createGFF3Feature("gene", SEQ_ID, 1L, seqLen + 1, Map.of());
        ValidationException ex = assertThrows(
                ValidationException.class,
                () -> check.validateFeatureEndWithinSequence(annotationWithFeature(feature), 1));
        assertTrue(ex.getMessage().contains("end position"));
    }

    @Test
    public void testValidateFeatureEndWithinSequenceLengthSuccess() throws Exception {
        long seqLen = 1000L;
        injectLookupReturning(SEQ_ID, seqLen);
        GFF3Feature feature = TestUtils.createGFF3Feature("gene", SEQ_ID, 1L, seqLen - 1, Map.of());
        assertDoesNotThrow(() -> check.validateFeatureEndWithinSequence(annotationWithFeature(feature), 1));
    ...
    @Test
    public void testValidateFeatureEndLookupThrowsIllegalState() throws Exception {
    ...
    @Test
    public void testValidateFeatureEndNoLookupSkipped() throws Exception {
        injectNoLookup();
